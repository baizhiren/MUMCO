...
def worker(remote, parent_remote, env_fn_wrapper):
    ...
    while True:
        ...
        if cmd == 'step':
            ob, reward, done, info, avi, object = env.step(data)
            if 'bool' in done.__class__.__name__:
                ...
                if np.all(done):
                    if settings.debug:
                        env.cal_statistic()
                    ob = env.reset()
            remote.send((ob, reward, done, info, avi, object))
        elif cmd == 'reset':
...
class SubprocVecEnv(ShareVecEnv):
    ...
    def step_wait(self):
        ...
        obs, rews, dones, infos, available_actions, objects = zip(*results)

        return np.stack(obs), np.stack(rews), np.stack(dones), infos, np.stack(available_actions), np.stack(objects)
...
class DummyVecEnv(ShareVecEnv):
    ...
    def step_wait(self):
        if settings.algorithm == 'rl' or settings.algorithm == 'OHM':
            ...
            results = [env.step_by_algorithm(algorithm=settings.algorithm) for env in self.envs]

        obs, rews, dones, infos, avis, _ = map(np.array, zip(*results))

        for (i, done) in enumerate(dones):
            if 'bool' in done.__class__.__name__:
                if done:
                    if settings.debug:
                        self.envs[i].cal_statistic()
                        self.envs[i].print_info()
                    obs[i] = self.envs[i].reset()
            else:
                if np.all(done):
                    self.envs[i].cal_statistic()
                    if settings.debug and (not settings.print_last_only_enable or settings.print_last_only):
                        self.envs[i].print_info()
                    obs[i] = self.envs[i].reset()

        self.actions = None
        return obs, rews, dones, infos, avis
    ...
    def print_info(self):
        self.envs[0].print_info()

    def print_step(self):
        self.envs[0].print_step()
